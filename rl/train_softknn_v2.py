...
MODEL_ID = {"06b": "Qwen/Qwen3-Embedding-0.6B",
            "8b": "Qwen/Qwen3-Embedding-8B"}[sys.argv[4] if len(sys.argv) > 4 else "06b"]
KL_BETA = float(sys.argv[5]) if len(sys.argv) > 5 else 0.0
...
for seed in seeds:
    ...
    t0 = time.time()
    tag = f"s{seed}_{mode}_{sys.argv[4] if len(sys.argv) > 4 else '06b'}_b{KL_BETA}"
    ...
    # reference policy for the KL anchor: the INIT (frozen-geometry) policy on train queries
    pi_ref = None
    if KL_BETA > 0:
        with torch.no_grad():
            E0 = encode(model, tr, grad=False)
            _, u0 = routing_terms(E0, E0, tr, torch.tensor(0.05, device=dev),
                                  LAMBDA_TRAIN, repo_mask_rows=grp_code[tr])
            pi_ref = F.softmax(u0 / 0.05, dim=1)
    ...
    for step in range(1, STEPS + 1):
        if mode == "lora":
            E_all = torch.zeros(len(qids), HID, device=dev)
        ...
        if pi_ref is not None:
            ...
            loss = loss + KL_BETA * (pi * (logpi - (pi_ref + 1e-9).log())).sum(dim=1).mean()
        opt.zero_grad()
        loss.backward()

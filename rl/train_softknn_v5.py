...
Usage: python train_softknn_v5.py <seeds-csv> <algo> <source> <beta> <outdir>
...
def hard_eval(E_q, E_m, mem_idx, q_idx, T, lam):
    with torch.no_grad():
        p = soft_vote(E_q, E_m, graded[:, mem_idx], T)
        u = p - lam * cost[:, mem_idx].median(dim=1).values[None, :]
...
for seed in seeds:
    ...
    t0 = time.time()
    tag = f"s{seed}_v5{algo}_{source}_b{KL_BETA}"
    ...
    for step in range(1, STEPS + 1):
        ...
        medc = src_cost[:, batch].median(dim=1).values
        u = p_hat - src_lam * medc[None, :]
        pi = F.softmax(u / tau, dim=1)
        R = (gm - src_lam * src_cost[:, batch]).T
    ...
    res = {"seed": seed, "mode": f"v5{algo}_{source}_b{KL_BETA}", "lam": pick["lam"],
